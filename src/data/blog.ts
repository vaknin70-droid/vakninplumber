// ====================================================
// 📝 קובץ הבלוג של ואקנין אינסטלציה
// ====================================================
// איך להוסיף מאמר חדש?
// 1. העתיקו אחד מהאובייקטים הקיימים
// 2. שנו את כל השדות
// 3. ודאו ש-slug הוא ייחודי (באנגלית, ללא רווחים, מקפים במקום רווחים)
// 4. שמרו — המאמר יופיע אוטומטית באתר
// ====================================================

export interface BlogPost {
  slug: string;           // מזהה ייחודי באנגלית, לדוג': "how-to-prevent-leaks"
  title: string;          // כותרת המאמר
  excerpt: string;        // תקציר קצר (2-3 משפטים)
  date: string;           // תאריך פרסום, לדוג': "12 באפריל 2026"
  category: string;       // קטגוריה, לדוג': "נזילות", "תחזוקה", "חירום"
  readTime: number;       // זמן קריאה משוער בדקות
  featured: boolean;      // true = יוצג בראש הבלוג
  content: {
    title: string;        // כותרת פסקה/חלק
    body: string;         // תוכן הפסקה
  }[];
  tips?: string[];        // טיפים מהירים (אופציונלי)
  faqs?: {
    question: string;
    answer: string;
  }[];
