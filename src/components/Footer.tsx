  ...
  return (
    <footer className="relative overflow-hidden bg-primary-dark py-12 pb-24 text-primary-foreground/80 md:pb-12">
      ...
      <div className="container relative z-10 pt-6">
          ...
          <div>
            ...
            <nav className="space-y-2 text-sm">
              {servicePages.slice(0, 6).map((s) => (
                <Link key={s.slug} to={`/services/${s.slug}`} className="block transition-colors hover:text-accent">
                  {s.name}
                </Link>
              ))}
            </nav>
          ...
          <div>
            ...
            <nav className="space-y-2 text-sm">
              {cityPages.slice(0, 6).map((c) => (
                <Link key={c.slug} to={`/city/${c.slug}`} className="block transition-colors hover:text-accent">
                  {`אינסטלטור ב${c.name}`}
                </Link>
              ))}
        ...
        <div className="mt-8 space-y-2 border-t border-primary-foreground/10 pt-6 text-center text-xs">
