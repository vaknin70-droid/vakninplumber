import { a1 as ROUTE_TYPE_HEADER, w as REROUTE_DIRECTIVE_HEADER, D as DEFAULT_404_COMPONENT, A as AstroError, Y as ActionNotFoundError, a8 as clientAddressSymbol, al as LocalsNotAnObject, am as FailedToFindPageMapSSR, an as REROUTABLE_STATUS_CODES, ac as responseSentSymbol, ao as nodeRequestAbortControllerCleanupSymbol } from './astro/server__d1cmUpD.mjs';
import colors from 'piccolore';
import 'clsx';
import { D as DEFAULT_404_ROUTE, c as default404Instance, e as ensure404Route } from './astro-designed-error-pages_D-I9MOIP.mjs';
...
import { N as NOOP_MIDDLEWARE_FN } from './noop-middleware_DEloUcDR.mjs';
...
function resolveClientDir(options) {
  ...
  const rel = path__default.relative(url.fileURLToPath(serverURLRaw), url.fileURLToPath(clientURLRaw));
  const serverFolder = path__default.basename(options.server);
...
async function readErrorPageFromDisk(client, status) {
  ...
  for (const filePath of filePaths) {
    const fullPath = path__default.join(client, filePath);
    try {
...
function createStaticHandler(app, options) {
  ...
  return (req, res, ssr) => {
    if (req.url) {
      ...
      const filePath = path__default.join(client, app.removeBase(urlPath));
